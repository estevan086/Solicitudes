			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("SolItem.SolicitudId") === oObject.SolItem.SolicitudId;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].SolItem.SolicitudId;
									} else {
										sCurrentId = aItemsNotInTheList[0].SolItem.SolicitudId;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SolHeader", {
										SolItem.SolicitudId : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("SolItem.SolicitudId"),
						title: oBindingContext.getProperty("TipoSol")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<SolHeader> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("TipoSol");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all SolHeader to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<SolHeader> (" + iExpectedLength + ")"}),
								success : function () {
